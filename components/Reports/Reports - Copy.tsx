...
const Reports: React.FC<ReportsProps> = ({ tasks, users }) => {
  ...
  const exportData = (format: string) => {
    const csvContent = "data:text/csv;charset=utf-8," 
      + ["نوع المهمة,الطالب,الوقت,التاريخ,المسؤول,التقييم", 
         ...filteredTasks.map(t => `${t.type},${t.studentName || ''},${t.duration},${t.createdAt},${t.username},${t.rating || ''}`)
        ].join("\n");
    
    const encodedUri = encodeURI(csvContent);
    const link = document.createElement("a");
    link.setAttribute("href", encodedUri);
    link.setAttribute("download", `report_${dateFilter}.${format}`);
      ...
      <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
          ...
          <div className="flex space-x-2 space-x-reverse">
            ...
            >
              <FileDown size={18} className="ml-2" /> تصدير Excel
            </button>
            <button 
              onClick={() => alert('PDF Export requires a PDF library. CSV/Excel is implemented.')}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-white border-b border-gray-100">
                <th className="py-4 px-6 text-right font-bold text-gray-500 uppercase tracking-wider">التاريخ</th>
                <th className="py-4 px-6 text-right font-bold text-gray-500 uppercase tracking-wider">المهمة</th>
                <th className="py-4 px-6 text-right font-bold text-gray-500 uppercase tracking-wider">المسؤول</th>
                <th className="py-4 px-6 text-right font-bold text-gray-500 uppercase tracking-wider">المدة</th>
                <th className="py-4 px-6 text-right font-bold text-gray-500 uppercase tracking-wider">التقييم</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {filteredTasks.map(t => (
                <tr key={t.id} className="hover:bg-indigo-50/30 transition-colors">
                  <td className="py-4 px-6 text-gray-600 font-medium">{new Date(t.createdAt).toLocaleDateString('ar-EG')}</td>
                  ...
                  <td className="py-4 px-6">
                    <div className="flex items-center">
                      <span className="text-amber-500 ml-1 font-bold">{t.rating || '-'}</span>
                    </div>
                  </td>
                </tr>
              ))}
